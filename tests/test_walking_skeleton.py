...
class WalkingSkeletonTest(unittest.TestCase):
    ...
    def installed_skill(self):
        ...
        with tempfile.TemporaryDirectory() as codex_home_value, tempfile.TemporaryDirectory() as workspace_value:
            ...
            yield added, installed_root, Path(workspace_value)
    ...
    def test_marketplace_install_exposes_one_invocable_skill(self) -> None:
        ...
        with self.installed_skill() as (added, installed_root, _workspace):
            ...
            self.assertTrue((skills[0].parent / "scripts" / "forge_video.py").is_file())

    def test_invocation_creates_collision_safe_forger_projects(self) -> None:
        with self.installed_skill() as (_added, installed_root, workspace):
            script = installed_root / "skills/forge-video/scripts/forge_video.py"
            first, _first_egress = invoke_under_egress_guard(script, workspace, "Neon Walk")
            ...
            second, _second_egress = invoke_under_egress_guard(script, workspace, "Neon Walk")
            ...
            self.assertEqual({path.name for path in (workspace / "forger-projects").iterdir()}, {"neon-walk", "neon-walk-2"})

    def test_isolated_workspace_harness_observes_project_state(self) -> None:
        with self.installed_skill() as (_added, installed_root, workspace):
            script = installed_root / "skills/forge-video/scripts/forge_video.py"
            result, _egress = invoke_under_egress_guard(script, workspace, "Neon Walk")
            project_path = Path(result["projectPath"])
            self.assertEqual({path.relative_to(project_path) for path in project_path.rglob("*") if path.is_file()}, {Path("forger.project.json")})
    ...
    def test_generation_boundary_harness_observes_no_final_video_job(self) -> None:
        with self.installed_skill() as (_added, installed_root, workspace):
            script = installed_root / "skills/forge-video/scripts/forge_video.py"
            result, egress = invoke_under_egress_guard(script, workspace, "Neon Walk")
            self.assertNotIn("generationEvents", result)
